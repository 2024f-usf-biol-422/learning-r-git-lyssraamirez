...
R packages.After the "Report.Rmd" file was completed it was kitted using `knitr` [@xie2023knitr] to configure a "Report.html" output report. "Chat GPT" [@openai2023chatgpt] was also used as a reference in code of figures.

### test
```{r subet-data}
print(params$snp_file)  # Should print the SNP file path
print(params$sample_id) # Should print the sample ID
file.exists(params$snp_file) # Should return TRUE
```

## Subsetting data
```{r test-data}
# Load the SNP data from the file specified in params
snp_data <- read.csv(params$snp_file, header = TRUE)

# Debugging: Check the first few rows and column names
print("Loaded SNP data:")
print(head(snp_data))
print("Column names:")
print(colnames(snp_data))

# Subset the data for the given sample ID
single_sample_data <- subset(snp_data, sample == params$sample_id)
